...
function buildCoaching(markers) {
  ...
  const testosterone = findMarker(markers, ["total testosterone", "testosterone"]);
  ...
  if (!notes.length) {
    notes.push(
      "Your bloodwork draft is ready for review. Confirm or edit any values that look off before saving."
    );
  }
...
export default function Home() {
  ...
  async function extractBloodworkDraft() {
    try {
      setMessage("");
      setDraftMarkers([]);
      setCoaching([]);
      ...
      if (normalized.length) {
        ...
        setMessage(
          `Draft ready. Review ${normalized.length} extracted markers, edit anything wrong, then click Confirm + Save Results.`
        );
      } else {
